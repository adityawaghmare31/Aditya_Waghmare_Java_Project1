...
package com.example.InternEmployeeManagementSystem.config;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests().antMatchers("/login/**").permitAll().and().authorizeRequests()
				...
				.antMatchers("/user/**").hasRole("EMPLOYEE") // Employee USER
				.antMatchers("/reports/**").hasRole("ADMIN").anyRequest().authenticated().and().httpBasic();
